...
class CMakeBuild(build_ext):
    ...
    def build_cmake(self, ext):
        ...
        cmake_args = ['-S' + os.path.join(current_dir, '.'), '-B' + str(build_temp)]

        self.announce('Configuring cmake project', level=3)
        self.spawn(['cmake'] + cmake_args)

        self.announce('Building binaries', level=3)
        self.spawn(['cmake', '--build', str(build_temp), '-j'])

        built_extensions = [
            os.path.join(build_temp, file)
            for file in os.listdir(build_temp)
            if os.path.isfile(os.path.join(build_temp, file))
            and os.path.splitext(file)[0].startswith(extension_path.stem)
            and os.path.splitext(file)[1] in ['.pyd', '.so']
        ]
        if not built_extensions:
            raise FileNotFoundError(f'No built extension found for {ext.name} in {build_temp}')
        shutil.copy2(built_extensions[0], extension_path)
...
def get_nccl_lib_name(base_dir):
    return _find_versioned_so(base_dir, 'libmccl')
...
if __name__ == '__main__':
    ...
    setuptools.setup(
        ...
        ext_modules=[
            setuptools.Extension('deep_ep._C', sources=[])
        ],
        cmdclass={
