...
import deep_ep
from utils import init_dist, bench, calc_diff, create_grouped_scores, inplace_unique, per_token_cast_to_fp8, per_token_cast_back, hash_tensor
...
def test_main(args: argparse.Namespace, num_sms: int,
    ...
    num_tokens_per_rdma_rank = torch.empty((num_nodes, ), dtype=torch.int, device='musa')
        ...
        for async_mode in (False, True):
            for current_x in (x_pure_rand, x, x_e4m3):
                for with_topk in (False, True):
                    ...
                    # # Test combine
                    bias_0 = torch.ones((num_tokens, hidden), dtype=torch.bfloat16, device='musa')
                    bias_1 = torch.randn((num_tokens, hidden), dtype=torch.bfloat16, device='musa')
    ...
    for current_x in (x_e4m3, x):
        ...
        if local_rank == 0:
            print(f'[tuning] Best dispatch ({"FP8" if isinstance(current_x, tuple) else "BF16"}): SMs {best_results[0]}, NVL chunk {best_results[1]}, RDMA chunk {best_results[2]}: {rdma_send_bytes / 1e9 / best_time:.2f} GB/s (RDMA), {nvl_recv_bytes / 1e9 / best_time:.2f} GB/s (NVL) {best_time}s', flush=True)
            print('', flush=True)

        if isinstance(current_x, tuple):
            # Gather FP8 the best config from rank 0
            best_dispatch_results = torch.tensor([best_results[0], best_results[1], best_results[2]], dtype=torch.int32, device='musa')
    ...
    if local_rank == 0:
        print(f'[tuning] Best combine: SMs {best_results[0]}, NVL chunk {best_results[1]}, RDMA chunk {best_results[2]}: {combine_bf16_rdma_recv_bytes / 1e9 / best_time:.2f} GB/s (RDMA), {combine_bf16_nvl_send_bytes / 1e9 / best_time:.2f} GB/s (NVL)  {best_time}s', flush=True)
        print('', flush=True)
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    buffer = deep_ep.Buffer(group, int(2e9), int(2e9), low_latency_mode=args.test_ll_compatibility,
                            num_qps_per_rank=num_qps_per_rank)
    assert num_local_ranks == 8 and num_ranks > 8
    torch.manual_seed(rank)

    if local_rank == 0:
        print("pressure_test_mode:", args.pressure_test_mode)
