...
class ElasticBuffer {
    ...
    // PP settings
    static constexpr int kPPDefaultNumSMs = 16;
...
public:
    ...
    void engram_write(const torch::Tensor& storage, const std::optional<torch::Tensor>& sf) {
        ...
        EP_HOST_ASSERT(storage.is_musa() and storage.is_contiguous());
        ...
        if (sf.has_value())
            EP_HOST_ASSERT(sf->dim() == 2 and sf->is_musa() and sf->is_contiguous() and
    ...
    engram_fetch(const torch::Tensor& indices, int num_qps, const bool& use_tma_aligned_col_major_sf) const {
        ...
        EP_HOST_ASSERT(indices.is_musa() and indices.is_contiguous());
    ...
    void pp_send(const torch::Tensor& x, const int& dst_rank_idx, const int& num_sms) const {
        ...
        EP_HOST_ASSERT(x.is_musa() and x.is_contiguous() and x.nbytes() <= num_max_pp_tensor_bytes);
        ...
        launch_pp_send(
            ...
            num_sms == 0 ? std::min(jit::device_runtime->get_num_sms(), kPPDefaultNumSMs) : num_sms,
            num_gpu_timeout_cycles,
    ...
    dispatch(const torch::Tensor& x,
        ...
        EP_HOST_ASSERT(x.is_musa() and x.is_contiguous());
        ...
        EP_HOST_ASSERT(topk_idx.is_musa() and topk_idx.is_contiguous());
        ...
        if (topk_weights.has_value()) {
            ...
            EP_HOST_ASSERT(topk_weights->is_musa() and topk_weights->is_contiguous());
        ...
        if (cumulative_local_expert_recv_stats.has_value()) {
            ...
            EP_HOST_ASSERT(cumulative_local_expert_recv_stats->is_musa() and
        ...
        if (cached_mode) {
            ...
            EP_HOST_ASSERT(num_local_experts == num_local_experts_);
            EP_HOST_ASSERT(psum_num_recv_tokens_per_expert.is_musa() and psum_num_recv_tokens_per_expert.is_contiguous());
        ...
        if (cached_mode) {
            ...
            EP_HOST_ASSERT(num_local_experts == num_local_experts_);
            EP_HOST_ASSERT(num_unaligned_recv_tokens_per_expert.is_musa() and num_unaligned_recv_tokens_per_expert.is_contiguous());
        ...
        if (cached_mode) {
            ...
            EP_HOST_ASSERT(psum_num_recv_tokens_per_scaleup_rank.is_musa() and psum_num_recv_tokens_per_scaleup_rank.is_contiguous());
        ...
        if (nccl_context->num_scaleout_ranks == 1) {
            if (cached_mode) {
                ...
                EP_HOST_ASSERT(dst_buffer_slot_idx.is_musa() and dst_buffer_slot_idx.is_contiguous());
        ...
        if (nccl_context->num_scaleout_ranks > 1) {
            ...
            if (cached_mode) {
                ...
                EP_HOST_ASSERT(token_metadata_at_forward->is_musa() and token_metadata_at_forward->is_contiguous());
            ...
            if (cached_mode) {
                ...
                EP_HOST_ASSERT(channel_linked_list->is_musa() and channel_linked_list->is_contiguous());
    ...
    combine(const torch::Tensor& x,
        ...
        const auto [num_scaleup_ranks] = get_shape<1>(psum_num_recv_tokens_per_scaleup_rank);
        EP_HOST_ASSERT(combined_topk_idx.is_musa() and combined_topk_idx.is_contiguous());
        ...
        EP_HOST_ASSERT(src_metadata.is_musa() and src_metadata.is_contiguous());
