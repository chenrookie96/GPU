...
#else
        EP_HOST_ASSERT(std::regex_search(output, match, std::regex(R"(mcc version (\d+\.\d+))")));
        std::sscanf(match[1].str().c_str(), "%d.%d", &major, &minor);
#endif
        return {major, minor};
    }
...
#else
        nvcc_path = cuda_home / "bin" / "mcc";
        const auto musa_wrapper_path = library_include_path / "musa";
        if (const auto env_nvcc_path = get_env<std::string>("EP_JIT_NVCC_COMPILER"); not env_nvcc_path.empty())
            nvcc_path = env_nvcc_path;
        const auto [nvcc_major, nvcc_minor] = get_nvcc_version();
        signature = fmt::format("MCC{}.{}", nvcc_major, nvcc_minor);

        // The override the compiler flags
        const auto [arch_major, arch_minor] = device_runtime->get_arch_pair();
        const auto arch = fmt::format("mp_{}{}", arch_major, arch_minor);
        flags = fmt::format("{} -I{} -I{} --offload-arch={} "
                            "-fPIC -O3 -Wno-deprecated-declarations -Wno-abi",
                            flags, library_include_path.c_str(), musa_wrapper_path.c_str(), arch);
#endif
    }
...
#else
        const auto command = fmt::format("cd {} && {} -x musa {} -fgpu-rdc --musa-device-only -fuse-ld=lld -o {} {}",
            compile_dir.c_str(), nvcc_path.c_str(), code_path.c_str(), cubin_path.c_str(), flags);
#endif
        if (get_env("EP_JIT_DEBUG", 0) or get_env("EP_JIT_PRINT_COMPILER_COMMAND", 0))
...
#else
            const auto ptx_command = fmt::format("cd {} && {} -x musa {} --musa-device-only -S -o {} {}",
                compile_dir.c_str(), nvcc_path.c_str(), code_path.c_str(), ptx_path->c_str(), flags);
#endif
            if (get_env("EP_JIT_DEBUG", 0) or get_env("EP_JIT_PRINT_COMPILER_COMMAND", 0))
