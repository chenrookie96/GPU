...
public:
    ...
    explicit KernelRuntime(const std::filesystem::path& dir_path) {
...
#if 1
        const auto [exit_code, symbols] = call_external_command(fmt::format("nm -g --defined-only {}", cubin_path.c_str()));
#else
        ...
        for (std::string line; std::getline(iss, line); ) {
            ...
            std::string address, type, symbol_name;
            line_iss >> address >> type >> symbol_name;
            if (symbol_name.empty() or type.size() != 1)
                continue;
            const auto symbol_type = type[0];
            if (symbol_type == 'W' &&
                not symbol_name.ends_with("_ConstCalcProg")) {
