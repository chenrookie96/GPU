...
static void* get_driver_handle() {
    ...
    if (handle == nullptr) {
        handle = dlopen("libmusa.so.1", RTLD_LAZY | RTLD_LOCAL);
        EP_HOST_ASSERT(handle != nullptr and "Failed to load CUDA driver `libcuda.so.1`");
    }
    return handle;
}

#define EP_STRINGIFY_IMPL(x) #x
#define EP_STRINGIFY(x) EP_STRINGIFY_IMPL(x)
...
static auto lazy_##name(Args&&... args) -> decltype(name(args...)) { \
    ...
    if (func == nullptr) { \
        func = reinterpret_cast<FuncType>(dlsym(get_driver_handle(), EP_STRINGIFY(name))); \
