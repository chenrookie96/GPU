...
#if USE_MUSA
    // Byte-swap each 32-bit half and then swap the halves.
    // Avoids b64 mov1 which mtxas cannot handle.
    uint32_t lo = static_cast<uint32_t>(x);
    uint32_t hi = static_cast<uint32_t>(x >> 32);
    uint32_t new_lo, new_hi;
    asm volatile(".mtx\n"
        ".reg .b32 ign;\n"
        "permute3 %0.b32, %1.b32, ign.b32, 0x0123;\n"
        : "=R"(new_lo) : "R"(hi));
    asm volatile(".mtx\n"
        ".reg .b32 ign;\n"
        "permute3 %0.b32, %1.b32, ign.b32, 0x0123;\n"
...
__device__ static __forceinline__ uint16_t HtoBE16(uint16_t x) {
    ...
    asm volatile(".mtx\n"
        ".reg .b32 ign;\n"
        "mov1 ign.b32, 0x0;\n"
        "permute3 %0.b32, %1.b32, ign.b32, 0x4401;\n"
        : "=R"(d)
        : "R"(a));
#else
    asm volatile(
...
__device__ static __forceinline__ void ibgda_update_dbr(nvshmemi_ibgda_device_qp_t* qp, uint32_t dbrec_head) {
    ...
    asm volatile(".mtx\n"
        ...
        ".reg .b32 ign;\n"
        "lop2.and dbrec_head_16b.b32, %1.b32, 0xffff;\n"
        "permute3 %0.b32, dbrec_head_16b.b32, ign.b32, 0x0123;\n"
        : "=R"(dbrec_val)
        : "R"(dbrec_head));
    __threadfence_system_noflush();
    atomicExch(dbrec_ptr, dbrec_val);
#else
    // This is equivalent to `WRITE_ONCE(dbrec_ptr, HtoBE32(dbrec_head & 0xffff))`
    asm("{\n\t"
        ".reg .b32 dbrec_head_16b;\n\t"
...
__device__ static __forceinline__ void ibgda_ring_db(nvshmemi_ibgda_device_qp_t* qp, uint16_t prod_idx) {
    ...
    __threadfence_system_noflush();
    atomicExch((unsigned long long *)bf_ptr, *(reinterpret_cast<uint64_t*>(&ctrl_seg)));
}
...
__device__ static __forceinline__ void ibgda_post_send(nvshmemi_ibgda_device_qp_t* qp, uint64_t new_prod_idx) {
    ...
    if (new_prod_idx > old_prod_idx) {
        // TODO(hongwei.liu): Figure out nics that need to update doorbell record
        // ibgda_update_dbr(qp, new_prod_idx);
        deep_ep::legacy::ibgda_ring_db(qp, new_prod_idx);
    }
...
__device__ static __forceinline__ void ibgda_submit_requests(nvshmemi_ibgda_device_qp_t* qp,
    ...
    if (!state->use_async_postsend) {
        // TODO(hongwei.liu): Figure out the best doorbell batch size according to latency perf model
        if (kAlwaysDoPostSend or (message_idx + 1) % state->num_requests_in_batch == 0)
            ibgda_post_send(qp, new_wqe_idx);
    }
}

__device__ static __forceinline__
void ibgda_post_send(nvshmemi_ibgda_device_qp_t *g_qp, nvshmemi_ibgda_device_qp_t* s_qp, uint64_t new_prod_idx) {
    nvshmemi_ibgda_device_qp_management_t *mvars = &g_qp->mvars;
    uint64_t old_prod_idx;
    ...
    if (new_prod_idx > old_prod_idx) {
        // TODO(hongwei.liu): Figure out nics that need to update doorbell record
        // ibgda_update_dbr(s_qp, new_prod_idx);
        deep_ep::legacy::ibgda_ring_db(s_qp, new_prod_idx);
    }
    ibgda_lock_release(&mvars->post_send_lock);
}
...
template <bool kAlwaysDoPostSend>
__device__ static __forceinline__
void ibgda_submit_requests(nvshmemi_ibgda_device_qp_t *g_qp, nvshmemi_ibgda_device_qp_t* s_qp, uint64_t base_wqe_idx,
    ...
    // Wait for prior WQE slots to be filled first
    unsigned long long int *ready_idx =
        (unsigned long long int *)(state->use_async_postsend ? s_qp->tx_wq.prod_idx
    ...
    if (!state->use_async_postsend) {
        // TODO(hongwei.liu): Figure out the best doorbell batch size according to latency perf model
        if (kAlwaysDoPostSend or (message_idx + 1) % state->num_requests_in_batch == 0)
            ibgda_post_send(g_qp, s_qp, new_wqe_idx);
    }
}
...
ibgda_get_lkey_and_rkey(uint64_t laddr, __be32* lkey, uint64_t raddr, int dst_pe, uint64_t* out_raddr, __be32* out_rkey, uint32_t dev_idx) {
    ...
    auto log2_mumem_granularity = state->log2_mumem_granularity;

    // Local key
    uint64_t idx = (laddr - heap_start) >> log2_mumem_granularity;
    ...
    *lkey = device_key.key;
    ...
    idx = ((roffset >> log2_mumem_granularity) * nvshmemi_device_state_d.npes) + dst_pe;
...
__device__ static __forceinline__ void ibgda_get_rkey(uint64_t addr, int dst_pe, uint64_t* out_raddr, __be32* out_rkey, uint32_t dev_idx) {
    ...
    uint64_t idx = ((roffset >> state->log2_mumem_granularity) * nvshmemi_device_state_d.npes) + dst_pe;
...
__device__ static __forceinline__ void nvshmemi_ibgda_rma_p(
    ...
    ibgda_get_rkey(reinterpret_cast<uint64_t>(rptr), dst_pe, &raddr, &rkey, 0);

    // Write WQEs
    uint64_t base_wqe_idx = ibgda_reserve_wqe_slots(qp, 1);
    void* wqe_ptrs;
    wqe_ptrs = deep_ep::legacy::ibgda_get_wqe_ptr(qp, base_wqe_idx);
    ibgda_write_rdma_write_inl_wqe(qp, reinterpret_cast<const uint32_t*>(&value), raddr, rkey, base_wqe_idx, &wqe_ptrs, imm);

    // Submit requests
    deep_ep::legacy::ibgda_submit_requests<true>(qp, base_wqe_idx, 1);
}
...
nvshmemi_ibgda_put_nbi_warp(uint64_t req_rptr, uint64_t req_lptr, size_t bytes, int dst_pe, int qp_id, int lane_id, int message_idx, nvshmemi_ibgda_device_qp_t *s_qp) {
    ...
    while (remaining_bytes > 0) {
        if (lane_id == num_wqes)
            my_chunk_size = min(remaining_bytes, ibgda_get_lkey_and_rkey(my_laddr = req_lptr, &my_lkey, req_rptr, dst_pe, &my_raddr, &my_rkey, 0));
    ...
    // Process WQE
    auto g_qp = ibgda_get_rc(dst_pe, qp_id);

    // base_wqe_idx = ibgda_reserve_wqe_slots(s_qp, num_wqes);
    uint64_t base_wqe_idx = 0;
    if (lane_id == 0)
        base_wqe_idx = ibgda_reserve_wqe_slots(g_qp, num_wqes);
    base_wqe_idx = __shfl_sync(0xffffffff, base_wqe_idx, 0);
    if (lane_id < num_wqes) {
        auto wqe_idx = base_wqe_idx + lane_id;
        auto wqe_ptr = deep_ep::legacy::ibgda_get_wqe_ptr(s_qp, wqe_idx);
        ibgda_write_rdma_write_wqe(s_qp, my_laddr, my_lkey, my_raddr, my_rkey, my_chunk_size, wqe_idx, &wqe_ptr);
    }
    __syncwarp();

    // Submit
    if (lane_id == 0)
        ibgda_submit_requests<kAlwaysDoPostSend>(g_qp, s_qp, base_wqe_idx, num_wqes, message_idx);
    __syncwarp();
}
...
__device__ static __forceinline__ void nvshmemi_ibgda_put_nbi_warp(
    ...
    while (remaining_bytes > 0) {
        if (lane_id == num_wqes) {
            my_chunk_size = min(remaining_bytes,
                                ibgda_get_lkey_and_rkey(my_laddr = req_lptr, &my_lkey, req_rptr, dst_pe, &my_raddr, &my_rkey, 0));
        }
    ...
    if (lane_id < num_wqes) {
        auto wqe_idx = base_wqe_idx + lane_id;
        auto wqe_ptr = deep_ep::legacy::ibgda_get_wqe_ptr(qp, wqe_idx);
...
__device__ __forceinline__ void nvshmemi_ibgda_amo_nonfetch_add_one_shot(void *rptr, const int& value, int pe, int qp_id, nvshmemi_ibgda_device_qp_t *s_qp) {
    nvshmemi_ibgda_device_qp_t *g_qp = ibgda_get_rc(pe, qp_id);

    __be32 rkey;
    uint64_t raddr;
    ibgda_get_rkey(reinterpret_cast<uint64_t>(rptr), pe, &raddr, &rkey, 0);

    uint64_t my_wqe_idx = ibgda_reserve_wqe_slots(g_qp, 1);
    void *wqe_ptrs = deep_ep::legacy::ibgda_get_wqe_ptr(s_qp, my_wqe_idx);

    ibgda_write_amo_add_wqe(s_qp, value, reinterpret_cast<uint64_t>(s_qp->ibuf.buf),
                            s_qp->ibuf.lkey, raddr, rkey, my_wqe_idx, &wqe_ptrs);

    // submit amo request by one-shot
    nvshmemi_ibgda_device_qp_management_t *mvars = &g_qp->mvars;

    uint64_t new_wqe_idx = my_wqe_idx + 1;
    auto state = ibgda_get_state();
    ...
    if (state->nic_buf_on_gpumem)
        ...
        __threadfence_system();
    // *ready_idx = new_wqe_idx;
    atomicExch(ready_idx, new_wqe_idx);

    // Always post, not in batch
    if (!state->use_async_postsend) {
        atomicExch((unsigned long long *)(&mvars->tx_wq.prod_idx), new_wqe_idx);
        // *reinterpret_cast<unsigned long long int*>(&mvars->tx_wq.prod_idx) = new_wqe_idx;

        auto bf_ptr = reinterpret_cast<uint64_t*>(s_qp->tx_wq.bf);
        ibgda_ctrl_seg_t ctrl_seg = {
            .opmod_idx_opcode = HtoBE32(new_wqe_idx << 8),
            .qpn_ds = HtoBE32(s_qp->qpn << 8)
        };
    
        EP_STATIC_ASSERT(sizeof(decltype(&ctrl_seg)) == sizeof(uint64_t), "");

        // WQE writes and ready idx update must be finished first
        // We don't need to concern ourselves with the scenario where the ready index is updated
        // but the WQE hasn't been written, as CQEs will only begin to be consumed after the NIC
        // doorbell is updated.
        __threadfence_system_noflush();
...
__device__ __forceinline__ void nvshmemi_ibgda_amo_nonfetch_add(
    ...
    if (is_local_copy) {
        ...
        void* wqe_ptrs = deep_ep::legacy::ibgda_get_wqe_ptr(qp, my_wqe_idx);

        ibgda_write_amo_add_wqe(qp, value, reinterpret_cast<uint64_t>(qp->ibuf.buf), qp->ibuf.lkey, raddr, rkey, my_wqe_idx, &wqe_ptrs);

        deep_ep::legacy::ibgda_submit_requests<true>(qp, my_wqe_idx, 1);
    }
}
...
__device__ __forceinline__ uint64_t nvshmemi_get_p2p_ptr(const uint64_t& ptr, const int& rank, const int& dst_rank) {
    ...
    if (rank == dst_rank) {
        ...
        if (peer_base != 0) {
            // MTLink connected
            ret = peer_base + (ptr - reinterpret_cast<uint64_t>(nvshmemi_device_state_d.heap_base));
        }
        // 0 means RDMA connected
    }
