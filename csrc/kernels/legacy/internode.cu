...
#if USE_MUSA
    return {
        (num_nvl_recv_buffer_tokens * (hidden_int4 * sizeof(int4) + num_scales * sizeof(float) + num_topk_idx * sizeof(int) + num_topk_weights * sizeof(float) + sizeof(SourceMeta)) * num_nvl_ranks *
         num_channels) /
            sizeof(int),
...
__global__ void __launch_bounds__(((kNumDispatchRDMASenderWarps + 2 + LEGACY_NUM_MAX_NVL_PEERS) * 32), 1)
    dispatch(int4* recv_x,
             ...
             int rank,
             int num_ranks) {
    enum class WarpRole { kRDMASender, kRDMASenderCoordinator, kRDMAAndNVLForwarder, kForwarderCoordinator, kNVLReceivers, kNVLReceiversCoordinator };
    ...
    const auto role_meta = [=]() -> std::pair<WarpRole, int> {
        if (is_forwarder) {
            ...
            return {WarpRole::kNVLReceiversCoordinator, 0};
        } else {
            return {WarpRole::kNVLReceivers, (warp_id + channel_id - kNumDispatchRDMASenderWarps - 1) % LEGACY_NUM_MAX_NVL_PEERS};
        }
    }();
    auto warp_role = role_meta.first;
    auto target_rank = role_meta.second;  // Not applicable for RDMA senders
    EP_DEVICE_ASSERT(num_warps == kNumDispatchRDMASenderWarps + 2 + LEGACY_NUM_MAX_NVL_PEERS);
    ...
    if (warp_role == WarpRole::kNVLReceivers or warp_role == WarpRole::kNVLReceiversCoordinator)
        rs_wr_buffer_ptr = buffer_ptrs[target_rank], ws_rr_buffer_ptr = buffer_ptrs[nvl_rank], rs_wr_rank = target_rank,
        ws_rr_rank = nvl_rank;

    // Allocate buffers
#if USE_MUSA
    auto nvl_channel_x = AsymBuffer<int4>(ws_rr_buffer_ptr, num_max_nvl_chunked_recv_tokens * hidden_int4, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels, rs_wr_rank).advance_also(rs_wr_buffer_ptr);
    auto nvl_channel_src_meta = AsymBuffer<SourceMeta>(ws_rr_buffer_ptr, num_max_nvl_chunked_recv_tokens, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels, rs_wr_rank).advance_also(rs_wr_buffer_ptr);
    auto nvl_channel_x_scales = AsymBuffer<float>(ws_rr_buffer_ptr, num_max_nvl_chunked_recv_tokens * num_scales, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels, rs_wr_rank).advance_also(rs_wr_buffer_ptr);
    auto nvl_channel_topk_idx = AsymBuffer<int>(ws_rr_buffer_ptr, num_max_nvl_chunked_recv_tokens * num_topk, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels, rs_wr_rank).advance_also(rs_wr_buffer_ptr);
    auto nvl_channel_topk_weights = AsymBuffer<float>(ws_rr_buffer_ptr, num_max_nvl_chunked_recv_tokens * num_topk, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels, rs_wr_rank).advance_also(rs_wr_buffer_ptr);
#else
...
#if USE_MUSA
    __musa::async_barrier bar_rdma_sender_smem = __musa::async_barrier(1);
    bar_rdma_sender_smem.init_arrival(kNumDispatchRDMASenderWarps + 1);
    __musa::async_barrier bar_forwarder_smem = __musa::async_barrier(2);
    bar_forwarder_smem.init_arrival(LEGACY_NUM_MAX_NVL_PEERS + 1);
    __musa::async_barrier bar_receiver_smem = __musa::async_barrier(3);
    bar_receiver_smem.init_arrival(LEGACY_NUM_MAX_NVL_PEERS + 1);
    __syncthreads_lm();
#endif
    ...
    __shared__ volatile int forward_cached_rdma_tail[kNumRDMARanks];
    auto sync_forwarder_smem = []() { asm volatile("barrier.sync 1, %0;" ::"r"((LEGACY_NUM_MAX_NVL_PEERS + 1) * 32)); };

    // Receiver warp synchronization
    __shared__ volatile int nvl_cached_tail[LEGACY_NUM_MAX_NVL_PEERS];
    __shared__ volatile bool nvl_receiver_retired[LEGACY_NUM_MAX_NVL_PEERS];
    auto sync_receiver_smem = []() { asm volatile("barrier.sync 2, %0;" ::"r"((LEGACY_NUM_MAX_NVL_PEERS + 1) * 32)); };
    ...
    if (warp_role == WarpRole::kRDMASender) {
...
#if USE_MUSA
        unsigned phase_id = bar_rdma_sender_smem.arrive();
        bar_rdma_sender_smem.wait(phase_id);
#else
        ...
        for (token_idx = token_start_idx + warp_id; token_idx < token_end_idx; token_idx += kNumDispatchRDMASenderWarps) {
            ...
            if (lane_id < kNumRDMARanks)
                is_token_in_rank_uint64 = *reinterpret_cast<const uint64_t*>(is_token_in_rank + token_idx * num_ranks + lane_id * LEGACY_NUM_MAX_NVL_PEERS);
...
#if USE_MUSA
        unsigned phase_id = bar_forwarder_smem.arrive();
        bar_forwarder_smem.wait(phase_id);
#else
        sync_forwarder_smem();
#endif
        ...
        while (__any_sync(0xffffffff, num_tokens_to_recv_from_rdma > 0)) {
            ...
            while (true) {
                ...
                if (__shfl_sync(0xffffffff, num_tokens_to_recv_from_rdma, src_rdma_rank) > 0) {
                    if (lane_id == src_rdma_rank and cached_rdma_channel_head == cached_rdma_channel_tail)
                        cached_rdma_channel_tail = forward_cached_rdma_tail[src_rdma_rank];
            ...
            for (int i = src_rdma_head, num_tokens_sent = 0; i < src_rdma_tail; ++i) {
                ...
                void* shifted = rdma_channel_data.recv_buffer(src_rdma_rank) + rdma_slot_idx * num_bytes_per_token;
        ...
        if (lane_id < kNumRDMARanks)
            forward_cached_rdma_tail[lane_id] = 0;
#if USE_MUSA
        unsigned phase_id = bar_forwarder_smem.arrive();
        bar_forwarder_smem.wait(phase_id);
#else
        ...
        while (true) {
            ...
            if (lane_id < kNumRDMARanks)
                forward_cached_rdma_tail[lane_id] = static_cast<int>(ld_acquire_sys_global(rdma_channel_tail.buffer(lane_id)));
    ...
    } else if (warp_role == WarpRole::kNVLReceiversCoordinator) {
        if (lane_id < LEGACY_NUM_MAX_NVL_PEERS) {
            nvl_cached_tail[lane_id] = 0;
            nvl_receiver_retired[lane_id] = false;
        }
#if USE_MUSA
        unsigned phase_id = bar_receiver_smem.arrive();
        bar_receiver_smem.wait(phase_id);
#else
        sync_receiver_smem();
#endif
        while (not __all_sync(0xffffffff, lane_id < LEGACY_NUM_MAX_NVL_PEERS ? nvl_receiver_retired[lane_id] : true)) {
            if (lane_id < LEGACY_NUM_MAX_NVL_PEERS and not nvl_receiver_retired[lane_id])
                nvl_cached_tail[lane_id] = ld_acquire_sys_global(nvl_channel_tail.buffer(lane_id));
            __nanosleep(LEGACY_NUM_WAIT_NANOSECONDS);
        }
        ...
        while (num_tokens_to_recv > 0) {
            ...
            while (true) {
                ...
                if (cached_channel_head_idx != cached_channel_tail_idx)
                    break;
                cached_channel_tail_idx = nvl_cached_tail[src_nvl_rank];
            ...
            for (int chunk_idx = 0; chunk_idx < num_recv_tokens; ++chunk_idx, --num_tokens_to_recv) {
                ...
                if (lane_id < num_topk) {
                    ...
                    auto idx_value = static_cast<topk_idx_t>(ld_nc_global(nvl_channel_topk_idx.buffer() + buffer_idx));
                    auto weight_value = ld_nc_global(nvl_channel_topk_weights.buffer() + buffer_idx);
        ...
        if (lane_id == 0)
            nvl_receiver_retired[src_nvl_rank] = true;
    }
...
#define DISPATCH_LAUNCH_CASE(num_rdma_ranks)                                                                                   \
    ...
    SETUP_LAUNCH_CONFIG(num_channels * 2, (kNumDispatchRDMASenderWarps + 2 + LEGACY_NUM_MAX_NVL_PEERS) * 32, stream);
...
__device__ __forceinline__ float bf16_element_as_float(const int4& value, int elem_idx) {
    unsigned int word;
    if (elem_idx < 2) {
        word = static_cast<unsigned int>(value.x);
    } else if (elem_idx < 4) {
        word = static_cast<unsigned int>(value.y);
    } else if (elem_idx < 6) {
        word = static_cast<unsigned int>(value.z);
    } else {
        word = static_cast<unsigned int>(value.w);
    }

    const auto shift = (elem_idx & 1) * 16;
    const auto raw = static_cast<unsigned int>((word >> shift) & 0xffffu);
...
__device__ int combine_token(bool is_token_in_rank,
    ...
    if constexpr (kUseTMA) {
        ...
        for (int i = lane_id; i < hidden_int4; i += 32) {
            ...
            // Reduce bias
            float values[kDtypePerInt4] = {0};
            EP_STATIC_ASSERT(sizeof(dtype_t) == sizeof(mt_bfloat16), "Only bf16 combine is supported");
            if constexpr (kMaybeWithBias) {
                #pragma unroll
                for (int j = 0; j < kDtypePerInt4; ++j)
                    values[j] = bf16_element_as_float(bias_0_value_int4, j) + bf16_element_as_float(bias_1_value_int4, j);
            }

            // Reduce all-to-all results
            // #pragma unroll
            for (int j = 0; j < num_topk_ranks; ++j) {
                auto recv_value_int4 = ld_nc_global(get_addr_fn(topk_ranks[j], slot_indices[j], i));
                #pragma unroll
                for (int k = 0; k < kDtypePerInt4; ++k)
                    values[k] += bf16_element_as_float(recv_value_int4, k);
            }
...
__global__ void __launch_bounds__((kNumRDMAReceivers + 1) * 32, 1) combine(int4* combined_x,
                                                                        float* combined_topk_weights,
    ...
    auto role_meta = [=]() -> std::pair<WarpRole, int> {
        ...
        if (is_forwarder_sm) {
            ...
            } else if (warp_id < LEGACY_NUM_MAX_NVL_PEERS + kNumForwarders) {
                auto shuffled_warp_id = warp_id - LEGACY_NUM_MAX_NVL_PEERS;
            ...
            if (warp_id < LEGACY_NUM_MAX_NVL_PEERS + kNumForwarders) {
...
#if USE_MUSA
    __musa::async_barrier bar_forwarder_smem = __musa::async_barrier(1);
    bar_forwarder_smem.init_arrival(kNumForwarders + 1);
    __musa::async_barrier bar_rdma_receiver_smem = __musa::async_barrier(2);
    bar_rdma_receiver_smem.init_arrival(kNumRDMAReceivers + 1);
    const int barCount = kNumForwarders / kNumWarpsPerForwarder;
    __musa::async_barrier bars_large_warp[barCount];
    for (int i = 0; i < barCount; ++i) {
      bars_large_warp[i] = __musa::async_barrier(i+3);
      bars_large_warp[i].init_arrival(kNumWarpsPerForwarder);
    }
    __syncthreads_lm();
#endif
    ...
    if (warp_role == WarpRole::kNVLSender) {
...
#if USE_MUSA
        auto nvl_channel_x = AsymBuffer<int4>(dst_buffer_ptr, num_max_nvl_chunked_recv_tokens * hidden_int4, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels, nvl_rank).advance_also(local_buffer_ptr);
        auto nvl_channel_src_meta = AsymBuffer<SourceMeta>(dst_buffer_ptr, num_max_nvl_chunked_recv_tokens, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels, nvl_rank).advance_also(local_buffer_ptr);
        auto nvl_channel_topk_weights = AsymBuffer<float>(dst_buffer_ptr, num_max_nvl_chunked_recv_tokens * num_topk, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels, nvl_rank).advance_also(local_buffer_ptr);
#else
...
#if USE_MUSA
        auto nvl_channel_x = AsymBuffer<int4>(local_nvl_buffer, num_max_nvl_chunked_recv_tokens * hidden_int4, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels).advance_also<LEGACY_NUM_MAX_NVL_PEERS>(nvl_buffers);
        auto nvl_channel_src_meta = AsymBuffer<SourceMeta>(local_nvl_buffer, num_max_nvl_chunked_recv_tokens, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels).advance_also<LEGACY_NUM_MAX_NVL_PEERS>(nvl_buffers);
        auto nvl_channel_topk_weights = AsymBuffer<float>(local_nvl_buffer, num_max_nvl_chunked_recv_tokens * num_topk, LEGACY_NUM_MAX_NVL_PEERS, channel_id, num_channels).advance_also<LEGACY_NUM_MAX_NVL_PEERS>(nvl_buffers);
#else
        auto nvl_channel_x =
            AsymBuffer<uint8_t>(
        ...
        __shared__ volatile int rdma_receiver_cached_rdma_tail[kNumRDMARanks];
        ...
        if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
            ...
            for (int token_start_idx = 0; token_start_idx < num_tokens_to_combine; token_start_idx += num_max_rdma_chunked_send_tokens) {
                ...
                if (kNumWarpsPerForwarder == 1) {
                    ...
                    unsigned phase_id = bars_large_warp[dst_rdma_rank].arrive();
                    bars_large_warp[dst_rdma_rank].wait(phase_id);
                }
#else
                sync_large_warp();
#endif
...
#endif
                    combine_token<LEGACY_NUM_MAX_NVL_PEERS, false, dtype_t, LEGACY_NUM_MAX_NVL_PEERS, false, kNumStages, kNumTMALoadBytes>(
                        expected_head >= 0,
                        expected_head,
                        lane_id,
                        hidden_int4,
            ...
            // Clean shared memory and sync
            EP_DEVICE_ASSERT(kNumRDMARanks <= 32);
            lane_id < kNumRDMARanks ? (rdma_receiver_cached_rdma_tail[lane_id] = 0) : 0;
            lane_id < kNumRDMARanks ? (rdma_receiver_rdma_head[warp_id][lane_id] = 0) : 0;
            lane_id == 0 ? (rdma_receiver_retired[warp_id] = false) : 0;
#if USE_MUSA
            unsigned phase_id = bar_rdma_receiver_smem.arrive();
            bar_rdma_receiver_smem.wait(phase_id);
#else
            ...
            for (int64_t token_idx = token_start_idx + warp_id; token_idx < token_end_idx; token_idx += kNumRDMAReceivers) {
                ...
                while (cached_channel_tail_idx <= expected_head) {
                    if (lane_id < kNumRDMARanks)
                        cached_channel_tail_idx = rdma_receiver_cached_rdma_tail[lane_id];
            ...
            while (true) {
                ...
                if (not is_forwarder_sm) {
                    if (lane_id < kNumRDMARanks)
                        rdma_receiver_cached_rdma_tail[dst_rdma_rank] = static_cast<int>(ld_acquire_sys_global(rdma_channel_tail.buffer(dst_rdma_rank)));
                    int min_head = std::numeric_limits<int>::max();
                    #pragma unroll
...
#define COMBINE_LAUNCH_CASE(num_rdma_ranks)                                           \
    ...
    SETUP_LAUNCH_CONFIG(num_channels * 2, (LEGACY_NUM_MAX_NVL_PEERS + num_forwarder_warps + 1) * 32, stream);
