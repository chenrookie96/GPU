...
#define NUM_MAX_WARP_GROUPS 32
#define NUM_MAX_EXPERTS_SHARED 720
template <bool kUseFP8, bool kUseUE8M0, int kHidden, int kNumWarpGroups=NUM_MAX_WARP_GROUPS, bool smallNumExperts = false>
__global__ __launch_bounds__(1024, 1) void dispatch(void* packed_recv_x,
    ...
    const auto num_expert_warps = num_warp_groups * num_warps_per_group;
    const bool is_expert_warp = warp_id < num_expert_warps;
    ...
    const int fixed_num_reduced_warps = 4;
    ...
    const size_t num_bytes_per_msg = align_up<size_t>(sizeof(int4) + (kUseFP8 ? (kHidden + num_scales * sizeof(float)) : (kHidden * sizeof(nv_bfloat16))), LEGACY_NUM_BUFFER_ALIGNMENT_BYTES);
    const size_t num_int4_per_msg = num_bytes_per_msg / sizeof(int4);
    EP_DEVICE_ASSERT(num_bytes_per_msg % sizeof(int4) == 0);

    // Expert counts
    constexpr int kNumMaxWarpGroups = kNumWarpGroups;
    __shared__ int shared_num_tokens_sent_per_expert[kNumMaxWarpGroups];

    __shared__ nvshmemi_ibgda_device_qp_t shared_qp_arr[NUM_MAX_EXPERTS_SHARED];
    if constexpr (smallNumExperts) {
        for (int expert_id = thread_id; expert_id < num_experts; expert_id += blockDim.x) {
            memcpy_global2shared(shared_qp_arr + expert_id, ibgda_get_state()->globalmem.rcs + expert_id, 1);
        }
    }

    __musa::async_barrier bar(1);
    bar.init_arrival(num_warps - fixed_num_reduced_warps);

    __musa::async_barrier reduce_bar(2);
    reduce_bar.init_arrival(fixed_num_reduced_warps);

    __musa::async_barrier bars[kNumMaxWarpGroups];

    #pragma unroll
    for (int i = 0; i < kNumMaxWarpGroups; ++i) {
          bars[i] = __musa::async_barrier(i + 3);
          bars[i].init_arrival(num_warps_per_group);
    }
    
    for (int i = thread_id; i < kNumMaxWarpGroups; i += blockDim.x) {
        shared_num_tokens_sent_per_expert[i] = 0;
    }
    __syncthreads_lm();
    ...
    if (warp_id < num_warps - fixed_num_reduced_warps) {
        constexpr int kNumElemsPerRead = sizeof(int4) / sizeof(nv_bfloat16);
        EP_STATIC_ASSERT(kHidden % (32 * kNumElemsPerRead) == 0, "Invalid hidden");
        EP_STATIC_ASSERT(kNumElemsPerRead * 32 % kNumPerChannels == 0, "Invalid vectorization");
        const auto num_threads = (num_warps - fixed_num_reduced_warps) * 32;
        ...
        for (int token_idx = sm_id; token_idx < num_tokens; token_idx += num_sms) {
            ...
            auto dst_expert_idx = warp_id < num_topk ? static_cast<int>(ldg(topk_idx + token_idx * num_topk + warp_id)) : -1;
                    ...
                    if (dst_p2p_ptr == 0) {
                        if constexpr (smallNumExperts)
                            nvshmemi_ibgda_put_nbi_warp(
                                dst_ptr, src_ptr, num_bytes_per_msg, dst_rank, dst_expert_local_idx, lane_id, slot_idx, &shared_qp_arr[dst_expert_idx]);
                        else
                            nvshmemi_ibgda_put_nbi_warp(
                        ...
                        UNROLLED_WARP_COPY(8, lane_id, num_int4_per_msg, dst_int4_ptr, src_int4_ptr, ld_normal, st_na_global);
                    }
                }

                // Increase counter after finishing
                __syncwarp();
                lane_id == 0 && dst_p2p_ptr != 0 ? __threadfence_system_noflush() : (void)0; // data and flag counter are separate
                lane_id == 0 ? atomic_add_release_global(atomic_finish_counter_per_expert + dst_expert_idx, 1) : 0;
            }
        }
    } else if (warp_id >= num_warps - fixed_num_reduced_warps) {
        EP_DEVICE_ASSERT(num_sms > 1);
        int reduce_warp_id = (warp_id - (num_warps - fixed_num_reduced_warps));
        if (sm_id == 0) {
            ...
            // #pragma unroll
            for (int i = lane_id + reduce_warp_id * 32; i < num_next_clean_int; i += fixed_num_reduced_warps * 32)
                next_clean[i] = 0;

            // Notify before executing `int_p`
            __syncwarp();
            // #pragma unroll
            for (int i = lane_id + reduce_warp_id * 32; i < num_experts; i += fixed_num_reduced_warps * 32)
        ...
        // Per lane count
        // #pragma unroll 8
        for (int i = lane_id + reduce_warp_id * 32; i < num_tokens * num_topk; i += fixed_num_reduced_warps * 32) {
            auto idx = static_cast<int>(ldg(topk_idx + i));
            if (idx >= expert_begin_idx and idx < expert_end_idx)
                atomicAdd(&shared_num_tokens_sent_per_expert[(idx - expert_begin_idx)], 1);
        }
        unsigned phase_id = reduce_bar.arrive();
        reduce_bar.wait(phase_id);

        // Warp reduce
        if (reduce_warp_id == 0 and lane_id == 0) {
            for (int i = expert_begin_idx; i < expert_end_idx; ++i) {
                auto sum = shared_num_tokens_sent_per_expert[i - expert_begin_idx];
    ...
    // Issue count sends
    if (is_expert_warp and responsible_expert_idx < num_experts and sub_warp_id == 0 and lane_id == 0) {
        ...
        if (not is_rank_masked(mask_buffer_ptr, dst_rank)) {
            if (dst_p2p_ptr == 0) {
                if constexpr (smallNumExperts)
                    nvshmemi_ibgda_amo_nonfetch_add_one_shot(reinterpret_cast<int*>(dst_ptr), -num_tokens_sent - 1, dst_rank, dst_expert_local_idx, &shared_qp_arr[responsible_expert_idx]);
                else
                    nvshmemi_ibgda_amo_nonfetch_add(reinterpret_cast<int*>(dst_ptr), -num_tokens_sent - 1, dst_rank, dst_expert_local_idx);
            } else {
                // all the stores done by memory_fence() in barrier_block()
...
LOW_LATENCY_DISPATCH_RECV:
    ...
    {
        int *sync_buffer = rdma_recv_count + num_experts;
        grid_barrier(num_sms, sync_buffer);
    }
    // cg::this_grid().sync();

    // Receiving and packing
    if (is_expert_warp and responsible_expert_idx < num_experts) {
...
#if USE_MUSA
        unsigned phase_id = bars[warp_group_id].arrive();
        bars[warp_group_id].wait(phase_id);
#else
        ...
        for (int i = sub_warp_id; i < num_recv_tokens; i += num_warps_per_group) {
            ...
            if (lane_id == 0)
                recv_src_info[recv_token_begin_idx + i] = ld_normal(src_src_idx);
            __syncwarp();
            ...
            UNROLLED_WARP_COPY(7, lane_id, hidden_int4, dst_data, src_data, ld_normal, st_na_global);
            ...
            if constexpr (kUseFP8) {
                ...
                if (lane_id < num_scales) {
                    ...
                    auto scale = extract_required_scale_format<kUseUE8M0>(ld_normal(src_scales + lane_id));
                    recv_x_scales[token_idx * token_stride + pack_idx * pack_stride + elem_idx] = scale;
                }
                if (lane_id + 32 < num_scales) {
                    const auto pack_idx = (lane_id + 32) / num_elems_per_pack;
                    const auto elem_idx = (lane_id + 32) % num_elems_per_pack;
                    auto scale = extract_required_scale_format<kUseUE8M0>(ld_normal(src_scales + lane_id + 32));
                    recv_x_scales[token_idx * token_stride + pack_idx * pack_stride + elem_idx] = scale;
                }
            }
        }
...
void dispatch(void* packed_recv_x,
    ...
    const auto num_sms = (phases & LEGACY_LOW_LATENCY_SEND_PHASE) ? num_device_sms : ceil_div(num_experts, num_warp_groups);
...
#define GET_FUNC_FOR_EXPERTS(kUseFP8, kUseUE8M0, kHidden, kNumWarpGroups)    \
    dispatch_func = num_experts > NUM_MAX_EXPERTS_SHARED ?                   \
        (dispatch<kUseFP8, kUseUE8M0, kHidden, kNumWarpGroups, false>) :     \
        (dispatch<kUseFP8, kUseUE8M0, kHidden, kNumWarpGroups, true>)

#define GET_FUNC_FOR_GROUPS(kUseFP8, kUseUE8M0, kHidden)                         \
    if (num_warp_groups <= 6) {                                                  \
        GET_FUNC_FOR_EXPERTS(kUseFP8, kUseUE8M0, kHidden, 6);                    \
    } else if (num_warp_groups <= 16) {                                          \
        GET_FUNC_FOR_EXPERTS(kUseFP8, kUseUE8M0, kHidden, 16);                   \
    } else if (num_warp_groups <= NUM_MAX_WARP_GROUPS) {                         \
        GET_FUNC_FOR_EXPERTS(kUseFP8, kUseUE8M0, kHidden, NUM_MAX_WARP_GROUPS);  \
    } else {                                                                     \
        EP_HOST_ASSERT(false && "Unsupported num_experts->num_warp_groups");     \
    }

#define GET_FUNC_FOR_FP8(kHidden)                          \
    auto dispatch_func = dispatch<false, false, kHidden>;  \
    if (use_fp8 and use_ue8m0) {                           \
        GET_FUNC_FOR_GROUPS(true, true, kHidden);          \
    } else if (use_fp8 and not use_ue8m0) {                \
        GET_FUNC_FOR_GROUPS(true, false, kHidden);         \
    } else {                                               \
        GET_FUNC_FOR_GROUPS(false, false, kHidden);        \
    }

#define DISPATCH_LAUNCH_CASE(hidden)                         \
    {                                                        \
        GET_FUNC_FOR_FP8(hidden)                             \
        LAUNCH_KERNEL(&cfg,                                  \
                      dispatch_func,                         \
                      packed_recv_x,                         \
    ...
    SETUP_LAUNCH_CONFIG(num_sms, 1024, stream);
...
template <bool kUseLogFMT, int kHidden, int kNumMaxTopk, int kNumMaxUnrolls, bool smallNumExperts = false>
__global__ __launch_bounds__(1024, 1) void combine(void* combined_x,
    ...
    constexpr size_t num_bytes_per_slot = align_up<size_t>(kHidden * sizeof(nv_bfloat16) + kNumMetaBytes, LEGACY_NUM_BUFFER_ALIGNMENT_BYTES);
    ...
    // Clean up next buffer that the dst_rank would signal for the first sm for each dst ranks, to notify before issuing finishing flag
    if (sub_warp_id == 0 and lane_id == 0) {
        const auto dispatch_src_rank = responsible_expert_idx / num_local_experts;
        const auto local_expert_idx = responsible_expert_idx % num_local_experts;
        next_clean[local_expert_idx * num_ranks + dispatch_src_rank] = 0;
        if (sm_id == 0 and warp_group_id == 0)
            next_clean[num_experts] = 0;
    }
    ...
    if (is_expert_warp and responsible_expert_idx < num_experts) {
        ...
        const auto layout = ldg(layout_range + local_expert_idx * num_ranks + dst_rank);
        const auto local_x =
        ...
        if (not is_rank_masked<true>(mask_buffer_ptr, dst_rank)) {
            for (int token_idx = offset + sub_warp_id; token_idx < offset + num_tokens_to_send; token_idx += num_warps_per_group) {
                ...
                const auto src_idx = ldg(local_src_info + token_idx);
...
#if USE_MUSA
                    UNROLLED_WARP_COPY(1, lane_id, hidden_bf16_int4, cpy_dst_int4_ptr, cpy_src_int4_ptr, ld_normal, st_na_global);
#else
                    // Prefetch
                    if (elect_one_sync())
                ...
                if (dst_p2p_ptr == 0) {
                  if constexpr (smallNumExperts)
                      nvshmemi_ibgda_put_nbi_warp(dst_ptr, buf_ptr, num_send_bytes, dst_rank, local_expert_idx, lane_id, token_idx - offset,
                                                  &shared_qp_arr[responsible_expert_idx]);
                  else
                      nvshmemi_ibgda_put_nbi_warp(dst_ptr, buf_ptr, num_send_bytes, dst_rank, local_expert_idx, lane_id, token_idx - offset);
                }
            }
    ...
    if (is_expert_warp and responsible_expert_idx < num_experts) {
        ...
        const auto local_expert_idx = responsible_expert_idx % num_local_experts;
        const auto global_expert_idx = rank * num_local_experts + local_expert_idx;
        uint64_t** full_barriers = nullptr;
        constexpr int kNumStages = 0;
#else
        ...
        if (sub_warp_id == 1 and lane_id == 0) {
            ...
            if (not is_rank_masked(mask_buffer_ptr, dst_rank)) {
                if (dst_p2p_ptr == 0) {
                    if constexpr (smallNumExperts)
                        nvshmemi_ibgda_amo_nonfetch_add_one_shot(reinterpret_cast<int*>(dst_ptr), 1, dst_rank, local_expert_idx, &shared_qp_arr[responsible_expert_idx]);
                    else
...
LOW_LATENCY_COMBINE_RECV:
    ...
    {
        int *sync_buffer = rdma_recv_flag + num_experts;
        grid_barrier(num_sms, sync_buffer);
    }
    // cg::this_grid().sync();
#if !USE_MUSA
    ...
    for (int hidden_idx = thread_id; hidden_idx < hidden_bf16_int4; hidden_idx += num_threads) {
        for (int token_idx = sm_id; token_idx < num_combined_tokens; token_idx += num_sms) {
            ...
            for (int i = 0; i < kNumMaxTopk; ++ i) {
                reg_topk_idx[i] = i < num_topk ? static_cast<int>(ldg(topk_idx + token_idx * num_topk + i)) : -1;
                reg_topk_weights[i] = i < num_topk ? __ldg(topk_weights + token_idx * num_topk + i) : -1;
            }

            float combined_values[kNumElemsPerInt4] = {0.0f};
            #pragma unroll
            for (int i = 0; i < kNumMaxTopk; ++ i) if (reg_topk_idx[i] >= 0) {
                // Read from sources
                ...
                // Reduce
                //  auto x_vec = ld_normal(reinterpret_cast<const int4*>(rdma_buffer_row) + hidden_idx);
                int4 x_vec = __lsu_ld_cache_hint((reinterpret_cast<const int4*>(rdma_buffer_row) + hidden_idx), 4, 2, 0, 0);
                const auto x_bf16 = reinterpret_cast<mt_bfloat16*>(&x_vec);
                combined_values[0] += static_cast<float>(x_bf16[0]) * reg_topk_weights[i];
                combined_values[1] += static_cast<float>(x_bf16[1]) * reg_topk_weights[i];
                combined_values[2] += static_cast<float>(x_bf16[2]) * reg_topk_weights[i];
                combined_values[3] += static_cast<float>(x_bf16[3]) * reg_topk_weights[i];
                combined_values[4] += static_cast<float>(x_bf16[4]) * reg_topk_weights[i];
                combined_values[5] += static_cast<float>(x_bf16[5]) * reg_topk_weights[i];
                combined_values[6] += static_cast<float>(x_bf16[6]) * reg_topk_weights[i];
                combined_values[7] += static_cast<float>(x_bf16[7]) * reg_topk_weights[i];
            }

            // Write results
            int4& combined_int4 = *reinterpret_cast<int4*>(combined_values);
            auto combined_bf16 = reinterpret_cast<mt_bfloat16*>(&combined_values);
            combined_bf16[0] = static_cast<mt_bfloat16>(combined_values[0]);
            combined_bf16[1] = static_cast<mt_bfloat16>(combined_values[1]);
            combined_bf16[2] = static_cast<mt_bfloat16>(combined_values[2]);
            combined_bf16[3] = static_cast<mt_bfloat16>(combined_values[3]);
            combined_bf16[4] = static_cast<mt_bfloat16>(combined_values[4]);
            combined_bf16[5] = static_cast<mt_bfloat16>(combined_values[5]);
            combined_bf16[6] = static_cast<mt_bfloat16>(combined_values[6]);
            combined_bf16[7] = static_cast<mt_bfloat16>(combined_values[7]);
            (static_cast<int4*>(combined_x) + token_idx * hidden_bf16_int4)[hidden_idx] = combined_int4; // asm correct
        }
    }
#endif
}
...
void combine(void* combined_x,
    ...
    const auto num_warps = num_warp_groups * num_warps_per_group;
    const auto num_sms = (phases & LEGACY_LOW_LATENCY_SEND_PHASE) ? num_device_sms :
...
#define COMBINE_LAUNCH_CASE(hidden)                                                                                                \
    {                                                                                                                              \
        auto combine_func = num_experts > NUM_MAX_EXPERTS_SHARED ?                                                                 \
          combine<false, hidden, kNumMaxTopk, kNumMaxUnrolls, false> : combine<false, hidden, kNumMaxTopk, kNumMaxUnrolls, true>;  \
