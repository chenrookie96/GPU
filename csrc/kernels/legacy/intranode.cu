...
#if USE_MUSA
    __musa::async_barrier bars[kNumRanks];
    for (int i = 0; i < kNumRanks; ++i) {
      bars[i] = __musa::async_barrier(i+1);
      bars[i].init_arrival(num_threads_per_rank / 32);
    }
    __syncthreads_lm();
#endif
    ...
    if (is_sender) {
        ...
        for (int64_t token_idx = token_start_idx; token_idx < token_end_idx;) {
            ...
            while (chunk_token_idx < num_max_send_tokens and token_idx < token_end_idx) {
                ...
                if (cached_channel_tail_idx % num_send_warps_per_rank == send_warp_id_in_rank) {
                    ...
                    UNROLLED_WARP_COPY(1, lane_id, hidden_int4, shifted_channel_x_buffers, shifted_x, ld_nc_global, st_volatile_global);
...
#if USE_MUSA
            unsigned phase_id = bars[responsible_rank].arrive();
            bars[responsible_rank].wait(phase_id);
#else
        ...
        while (num_tokens_to_recv > 0) {
            ...
            for (int chunk_idx = recv_warp_id_in_rank; chunk_idx < num_recv_tokens; chunk_idx += num_recv_warps_per_rank) {
                ...
                __syncwarp();
#else
                UNROLLED_WARP_COPY(1, lane_id, hidden_int4, shifted_recv_x_int4, shifted_buffer_x_int4, ld_nc_global, st_na_global);
#endif
            }
...
#if USE_MUSA
            phase_id = bars[responsible_rank].arrive();
            bars[responsible_rank].wait(phase_id);
#else
            asm volatile("bar.sync %0, %1;" ::"r"(responsible_rank), "r"(num_threads_per_rank));
#endif
    ...
    if (is_sender) {
        ...
        for (int64_t token_idx = token_start_idx; token_idx < token_end_idx;) {
            ...
            for (int i = send_warp_id_in_rank; i < num_round_tokens; i += num_send_warps_per_rank) {
                ...
                UNROLLED_WARP_COPY(1, lane_id, hidden_int4, shifted_x_buffers, shifted_x, ld_nc_global, st_volatile_global);
            ...
            // Move tail index
#if USE_MUSA
            unsigned phase_id = bars[send_rank_id].arrive();
            bars[send_rank_id].wait(phase_id);
#else
        ...
        __shared__ volatile int channel_tail_idx[32];
        ...
        if (thread_id < 32) {
            ...
            for (int64_t token_idx = token_start_idx + recv_warp_id - 1; token_idx < token_end_idx; token_idx += num_recv_warps - 1) {
                ...
                for (int i = 0; i < kNumRanks; ++i) {
                    ...
                    slot_indices[i] = (expected_head_i >= 0) ? expected_head_i % num_recv_buffer_tokens : -1;
                }
                ...
                for (int i = lane_id; i < hidden_int4; i += 32) {
                    ...
                    for (int j = 0; j < kNumRanks; ++j) {
                        if (slot_indices[j] != -1) {
                            int4 recv_value_int4 = ld_nc_global(channel_x_buffers[j].buffer() + slot_indices[j] * hidden_int4 + i);
                            auto recv_value_dtypes = reinterpret_cast<const dtype_t*>(&recv_value_int4);
                            #pragma unroll
                ...
                if (lane_id < num_topk) {
                    ...
                    #pragma unroll
                    for (int i = 0; i < kNumRanks; ++i) {
                        value += (slot_indices[i] != -1) ? ld_nc_global(channel_topk_weights_buffers[i].buffer() + slot_indices[i] * num_topk + lane_id) : 0;
                    }
