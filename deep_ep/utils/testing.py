...
def flush_l2_cache(enabled: bool = True):
    ...
    if enabled:
        torch.empty(int(l2_flush_cache_size // 4), dtype=torch.int, device='musa').zero_()
...
def bench_kineto(fn,
    ...
    with suppress():
        ...
        profiler = torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.MUSA], schedule=schedule, acc_events=True)
        dummy = torch.ones(1, dtype=torch.float, device='musa')
        with profiler:
            for i in range(2):
                for _ in range(num_tests):
                    ...
                    if barrier_comm_profiling:
                        torch.musa._sleep(int(2e7)) # ~10ms
    ...
    prof_lines = profiler.key_averages().table(sort_by='count', max_name_column_width=100).split('\n')
