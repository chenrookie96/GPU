...
def index_rows(t: torch.Tensor, indices) -> torch.Tensor:
    if t.device.type == 'musa' and t.dtype == torch.float8_e4m3fn:
        indexed = t.contiguous().view(torch.uint8)[indices].contiguous().view(torch.float8_e4m3fn)
        return indexed
    return t[indices]


def equal_values(lhs: torch.Tensor, rhs: torch.Tensor) -> torch.Tensor:
    if lhs.device.type == 'musa' and lhs.dtype == torch.float8_e4m3fn:
        return lhs.contiguous().view(torch.uint8) == rhs.contiguous().view(torch.uint8)
    return lhs == rhs
...
def dispatch(x: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
    ...
    for dst_rank_idx in range(num_ranks):
        ...
        x_to_send = index_rows(x, indices_to_send)
        sf_to_send = index_rows(sf, indices_to_send) if use_fp8 else None
