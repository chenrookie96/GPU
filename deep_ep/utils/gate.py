...
def generate_topk_idx(rank_count: torch.Tensor, num_tokens: int, num_experts: int, num_ranks: int, num_topk: int) -> torch.Tensor:
    ...
    """
    assert torch.equal(torch.sum(rank_count, dim=1), torch.ones(num_tokens, dtype=torch.int, device='musa') * num_topk)
    assert (num_tokens, num_ranks) == rank_count.shape
    num_experts_per_rank = num_experts // num_ranks

    # Generate base value
    base_vals = torch.arange(num_experts, device='musa').view(1, num_ranks, num_experts_per_rank).expand(num_tokens, num_ranks, num_experts_per_rank)

    # Randomize the ordering within each row
    rand_vals = torch.rand(num_tokens, num_ranks, num_experts_per_rank, device='musa')
    perm_indices = torch.argsort(rand_vals, dim=-1)
    permuted = torch.gather(base_vals, 2, perm_indices)

    # Create the mask
    k_idx = torch.arange(num_experts_per_rank, device='musa').view(1, 1, num_experts_per_rank).expand(num_tokens, num_ranks, num_experts_per_rank)
...
def generate_rank_count(num_tokens: int, num_experts: int, num_ranks: int, num_topk: int, ratio: float) -> torch.Tensor:
    ...
    random_scores = torch.rand(num_tokens, num_experts, device='musa')
    ...
    optional_token_indices = optional_token_indices[torch.randperm(num_tokens - must_count, device='musa')][:special_token_count - must_count]
    ...
    rank_perm = (torch.randperm(num_normal_ranks, device='musa') + 1).repeat(num_tokens * num_topk // num_normal_ranks + 1)

    # Compute cumulative sum of a to get starting indices in b for each row
    a_cumsum = torch.cumsum(torch.cat((torch.tensor([0], device='musa'), a)), dim=0)
    ...
    rank_perm_with_special_rank = torch.zeros(num_tokens * num_topk, dtype=torch.long, device='musa')  # (n * k,)
    special_token_mask = torch.zeros(num_tokens * num_topk, dtype=torch.bool, device='musa')
    ...
    all_indices = torch.arange(num_tokens * num_topk, device='musa')
    ...
    col_idx = torch.arange(num_topk, device='musa').view(1, num_topk)  # (1, num_topk)
    ...
    shuffle_indices = torch.randperm(num_tokens, device='musa')
    result = result[shuffle_indices]  # Shuffle rows

    # Create rank count tensor
    rank_count = torch.zeros((num_tokens, num_ranks), dtype=torch.int32, device='musa')
...
def get_precise_unbalanced_scores(num_tokens: int, num_experts: int, num_ranks: int, num_topk: int, ratio: float):
    ...
    threshold = 0.9
    scores = torch.empty((num_tokens, num_experts), dtype=torch.float32, device='musa')
    ...
    topk_scores = torch.empty((num_tokens, num_topk), dtype=torch.float32, device='musa')
