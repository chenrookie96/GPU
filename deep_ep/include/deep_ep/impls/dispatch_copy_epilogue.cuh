...
dispatch_copy_epilogue_impl(void* buffer, void* workspace,
 ...
 // Utils
    ...
    for (int i = global_warp_idx; i < num_recv_tokens; i += kNumWarps * kNumSMs) {
        ...
        if constexpr (kDoCreateLinkedList) {
            if (ptx::elect_one_sync())
                channel_linked_list[buffer_token.get_linked_list_idx_ptr()[master_src_topk_idx]] = i;
            __syncwarp();
        }

        // Store data
        auto copy_mask = kDoExpand ? ptx::gather(dst_tensor_idx >= 0) : 1;
        while (copy_mask) {
            const int valid_lane_idx = __ffs(copy_mask) - 1;
            const auto dst_idx = ptx::exchange(dst_tensor_idx, valid_lane_idx);
            EP_STATIC_ASSERT(kNumHiddenBytes % sizeof(int4) == 0, "Unaligned hidden bytes");
            const auto src_hidden_ptr = reinterpret_cast<const int4*>(buffer_token.get_hidden_ptr());
            auto dst_hidden_ptr = reinterpret_cast<int4*>(
                math::advance_ptr(recv_x, static_cast<int64_t>(dst_idx) * kNumHiddenBytes));

            for (int k = lane_idx; k < kNumHiddenBytes / static_cast<int>(sizeof(int4)); k += 32)
                dst_hidden_ptr[k] = ptx::ldg(src_hidden_ptr + k);
            copy_mask ^= 1 << valid_lane_idx;
        }
        __syncwarp();
        ...
        if constexpr (kNumSFPacks > 0) {
            ...
            // Load into registers
            const auto src_sf_ptr = buffer_token.get_sf_ptr();
            sf_pack_t reg_src[kNumFullIters + 1];
            #pragma unroll
            for (int k = 0; k < kNumFullIters; ++ k)
                reg_src[k] = src_sf_ptr[k * 32 + lane_idx];
            if (do_last_iter)
                reg_src[kNumFullIters] = src_sf_ptr[kNumFullIters * 32 + lane_idx];
        ...
        // Store the top-k weights
        if (kDoExpand and recv_topk_weights != nullptr and dst_tensor_idx >= 0) {
            recv_topk_weights[dst_tensor_idx] = buffer_token.get_topk_weights_ptr()[lane_idx];
        } else if (not kDoExpand and recv_topk_weights != nullptr and lane_idx < kNumTopk) {
            // For backward, weights are optional
            recv_topk_weights[i * kNumTopk + lane_idx] = buffer_token.get_topk_weights_ptr()[lane_idx];
        }
        __syncwarp();
        ...
        if constexpr (not kCachedMode) {
            if (ptx::elect_one_sync()) {
                recv_src_metadata[i * kMetadataStride + 0] = *buffer_token.get_src_token_global_idx_ptr();
                if constexpr (kNumScaleoutRanks == 1) {
                    recv_src_metadata[i * kMetadataStride + 1] = current_rank_idx * kNumTopk + master_src_topk_idx;
                } else {
                    recv_src_metadata[i * kMetadataStride + 1] = (i - current_rank_start) * kNumTopk + master_src_topk_idx;
