...
constexpr int kNumExpertsPerRank = kNumExperts / kNumRanks;
    ...
    ptx::named_barrier_init<kNumNotifyThreads>(kNotifyBarrierIndex);
    ...
    if (warp_idx < kNumNotifyWarps) {
        ...
        for (int token_idx = token_start; token_idx < num_tokens; token_idx += token_stride) {
            const auto token_i64_idx = static_cast<int64_t>(token_idx);
            EP_STATIC_ASSERT(kNumHiddenBytes % sizeof(int4) == 0, "Unaligned hidden bytes");
            const auto src_hidden_ptr = reinterpret_cast<const int4*>(math::advance_ptr(x, token_i64_idx * kNumHiddenBytes));
            const auto write_token = [&](auto&& token_buffer) {
                auto dst_hidden_ptr = reinterpret_cast<int4*>(token_buffer.get_hidden_ptr());

                for (int i = lane_idx; i < kNumHiddenBytes / static_cast<int>(sizeof(int4)); i += 32)
                    dst_hidden_ptr[i] = ptx::ldg(src_hidden_ptr + i);

                if constexpr (kNumSFPacks > 0) {
                    const auto src_sf_ptr = math::advance_ptr<sf_pack_t>(sf, token_i64_idx * sf_token_stride * sizeof(sf_pack_t));
                    auto dst_sf_ptr = token_buffer.get_sf_ptr();
                    for (int i = lane_idx; i < kNumSFPacks; i += 32)
                        dst_sf_ptr[i] = src_sf_ptr[i * sf_hidden_stride];
                }

                if (lane_idx < kNumTopk) {
                    const auto uncasted_dst_expert_idx = __ldg(topk_idx + token_idx * kNumTopk + lane_idx);
                    const auto dst_expert_idx = static_cast<int>(uncasted_dst_expert_idx);
                    token_buffer.get_topk_idx_ptr()[lane_idx] = dst_expert_idx;
                    if (topk_weights != nullptr)
                        token_buffer.get_topk_weights_ptr()[lane_idx] = __ldg(topk_weights + token_idx * kNumTopk + lane_idx);
                    if (copied_topk_idx != nullptr)
                        copied_topk_idx[token_idx * kNumTopk + lane_idx] = uncasted_dst_expert_idx;
                }

                if (ptx::elect_one_sync())
                    *token_buffer.get_src_token_global_idx_ptr() = rank_idx * kNumMaxTokensPerRank + token_idx;
            };
            ...
            auto send_token_buffer = send_buffer.get_token_buffer(token_idx);
            if constexpr (not kIsScaleupNVLink)
                write_token(send_token_buffer);

            const auto send_dst_mask = ptx::gather(stored_dst_slot_idx >= 0);
            int remaining_send_dst_mask = send_dst_mask;
            while (remaining_send_dst_mask != 0) {
                const auto sender_lane_idx = ptx::ffs(remaining_send_dst_mask);
                const auto dst_slot_idx = ptx::exchange(stored_dst_slot_idx, sender_lane_idx);
                const auto dst_rank_idx = ptx::exchange(stored_dst_rank_idx, sender_lane_idx);
                const auto dst_ptr = gin.get_sym_ptr<team_t>(
                    recv_buffer.get_token_buffer(dst_slot_idx).get_base_ptr(), dst_rank_idx);

                if (dst_ptr != nullptr) {
                    auto recv_token_buffer = recv_buffer.get_token_buffer(dst_slot_idx);
                    recv_token_buffer.set_base_ptr(dst_ptr);
                    write_token(recv_token_buffer);
                }
                __syncwarp();

                if constexpr (not kIsScaleupNVLink) {
                    if (dst_ptr == nullptr and ptx::elect_one_sync()) {
                        gin.put<team_t>(recv_buffer.get_token_buffer(dst_slot_idx).get_base_ptr(),
                                        send_token_buffer.get_base_ptr(), send_token_buffer.get_num_bytes<false>(), dst_rank_idx);
                    }
                    __syncwarp();
                }

                remaining_send_dst_mask ^= 1u << sender_lane_idx;
            }
        }
    }
