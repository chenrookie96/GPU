...
struct WorkspaceLayout {
    ...
    static constexpr int64_t kNumGridBarrierSignalBytes = 8;
    ...
    static int64_t get_num_bytes() {
        ...
        // Local grid barrier signals
        num_bytes += kNumGridBarrierSignalBytes;
    ...
    __forceinline__ __device__ __host__ int* get_grid_barrier_counter_ptr() const {
        return math::advance_ptr<int>(workspace, kNumBarrierSignalBytes);
    }

    __forceinline__ __device__ __host__ int* get_grid_barrier_phase_ptr() const {
        return math::advance_ptr<int>(workspace, kNumBarrierSignalBytes + sizeof(int));
    }

    __forceinline__ __device__ __host__ int64_t* get_notify_reduction_workspace_ptr() const {
        return math::advance_ptr<int64_t>(workspace, kNumBarrierSignalBytes + kNumGridBarrierSignalBytes);
    }
