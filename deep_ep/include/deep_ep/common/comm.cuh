...
__forceinline__ __device__ void local_grid_barrier(const layout::WorkspaceLayout& workspace) {
#if USE_MUSA
    const auto thread_idx = static_cast<int>(threadIdx.x);
    const auto num_blocks = static_cast<int>(gridDim.x);
    __syncthreads();
    if (thread_idx == 0) {
        auto counter_ptr = workspace.get_grid_barrier_counter_ptr();
        auto phase_ptr = workspace.get_grid_barrier_phase_ptr();
        const auto phase = ptx::ld_volatile<int>(phase_ptr);
        __threadfence_system();
        const auto count = atomicAdd(counter_ptr, 1);
        if (count == num_blocks - 1) {
            *counter_ptr = 0;
            __threadfence_system();
            atomicAdd(phase_ptr, 1);
        } else {
            timeout_while<kNumTimeoutCycles>([=](const bool&) {
                return ptx::ld_volatile<int>(phase_ptr) != phase;
            });
        }
    }
    __syncthreads();
...
__forceinline__ __device__ void gin_barrier_wo_local_sync(
    ...
    if constexpr (kFlushStores) {
        ...
        (gridDim.x > 1) ? local_grid_barrier<kNumTimeoutCycles>(workspace) : __syncthreads();
    }
...
__forceinline__ __device__ void scaleup_barrier_wo_local_sync(
    ...
    if constexpr (kIsScaleupNVLink) {
        ...
        gin_barrier_wo_local_sync<kNumRanks, kNumSMs, kNumThreads, kNumQPs, kNumTimeoutCycles, ncclTeamTagWorld, kTag, kFlushStores>(
            gin.nccl_dev_comm, workspace, 1, rank_idx, sm_idx, thread_idx);
    }
}
...
__forceinline__ __device__ void scaleout_barrier_wo_local_sync(
    ...
    gin_barrier_wo_local_sync<kNumRanks, kNumSMs, kNumThreads, kNumQPs, kNumTimeoutCycles, ncclTeamTagRail, kTag, kFlushStores>(
        gin.nccl_dev_comm, workspace, scaleout_rank_idx, scaleup_rank_idx, sm_idx, thread_idx);
}
...
__forceinline__ __device__ void gpu_barrier(const handle::NCCLGin& gin,
    ...
    if constexpr (kSyncAtStart) {
        local_grid_barrier<kNumTimeoutCycles>(workspace);
    } else {
