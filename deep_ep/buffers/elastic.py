...
class EPHandle:
    ...
    def deterministic_sort(self,
        ...
        if not self.do_expand:
            ...
            token_idx2expert_idx = torch.bucketize(torch.arange(tensor_dim0_after_expand, device='musa'),
...
class ElasticBuffer:
    ...
    def __init__(self,
        ...
        if num_cpu_bytes > 0:
            ...
            num_total_gpu_bytes = torch.musa.get_device_properties('musa').total_memory
    ...
    def barrier(self, use_comm_stream: bool = True, with_cpu_sync: bool = False, sequential: bool = True) -> None:
        ...
        Arguments:
            ...
            with_cpu_sync: whether to also call `musaDeviceSynchronize` before and after the barrier.
    ...
    def pp_send(self, t: torch.Tensor, dst_rank_idx: int, num_sms: int = 0) -> None:
        ...
        Arguments:
            ...
            num_sms: the number of SMs to use (0 for the default configured by the runtime).
        """
    ...
    def get_theoretical_num_sms(self, num_experts: int, num_topk: int,
        ...
        num_device_sms = torch.musa.get_device_properties('musa').multi_processor_count
