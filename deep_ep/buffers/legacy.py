...
class Buffer:
    ...
    def __init__(self,
                 ...
                 use_ace: bool = False,
                 num_ace_buffers: int = 1,
                 token_num: int = 0,
                 hidden_size: int = 0,
                 num_topk: int = 0) -> None:  # noqa: F821
        """
        ...
        Arguments:
            ...
            use_ace: whether to use ace for intranode, only support ep <= 8 in one node
        """
        ...
        self.runtime = _C.Buffer(self.rank, self.group_size, num_nvl_bytes, num_rdma_bytes, low_latency_mode,
                                 explicitly_destroy, enable_shrink, allow_mnnvl, use_ace, num_ace_buffers, token_num, hidden_size, num_topk)
        ...
        if self.runtime.get_num_rdma_ranks() > 1 or low_latency_mode:
            ...
            os.environ['NVSHMEM_IBGDA_FORCE_NIC_BUF_MEMTYPE'] = os.environ.get('NVSHMEM_IBGDA_FORCE_NIC_BUF_MEMTYPE', 'hostmem')
            os.environ['NVSHMEM_IBGDA_NIC_HANDLER'] = os.environ.get('NVSHMEM_IBGDA_NIC_HANDLER', 'gpu')
            os.environ['NVSHMEM_IBGDA_NUM_REQUESTS_IN_BATCH'] = os.environ.get('NVSHMEM_IBGDA_NUM_REQUESTS_IN_BATCH', '4')
            ...
            os.environ['NVSHMEM_MUMEM_GRANULARITY'] = f'{2 ** 29}'
        ...
        ace_unique_id = None
        if self.use_ace == True:
            # init ace comm
            ace_unique_ids = [None, ] * self.group_size
            if self.rank == 0:
                ace_unique_id = self.runtime.get_ace_unique_id()
            dist.all_gather_object(ace_unique_ids, ace_unique_id, group)
            ace_unique_id = ace_unique_ids[0]
        # Make CPP runtime available
        self.runtime.sync(device_ids, ipc_handles, root_unique_id, ace_unique_id)
    ...
    def capture() -> EventOverlap:
        """
        Capture a CUDA event on the current stream, i.e. `torch.musa.current_stream()`.
    ...
    def get_comm_stream(self) -> torch.Stream:
        ...
        return torch.musa.Stream(stream_id=ts.stream_id, device_index=ts.device_index, device_type=ts.device_type)
    ...
    def get_ace_combine_buffer(self, num_out_tokens: int, hidden: int, num_topk :int, with_topk: bool, buffer_index:int = 0):
        return self.runtime.get_ace_combine_buffer(num_out_tokens, hidden, num_topk, with_topk, buffer_index)
    ...
    def dispatch_ace(self, x: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
                     handle: Optional[Tuple] = None,
                     topk_idx: Optional[torch.Tensor] = None, topk_weights: Optional[torch.Tensor] = None,
                     num_tokens_per_rank: Optional[torch.Tensor] = None, num_tokens_per_expert: Optional[torch.Tensor] = None,
                     expert_alignment: int = 1, previous_event: Optional[EventOverlap] = None, async_finish: bool = False, allocate_on_comm_stream: bool = False, buffer_idx: int = 0):
        """
        Dispatch tokens to different ranks using ACE.
        All preprocessing (routing, permute) is done in C++ for better performance.
        """
        x, x_scales = x if isinstance(x, tuple) else (x, None)
        if handle is not None:
            assert topk_idx is None and topk_weights is None
            global_stride_list, row_id_map_non_trans, num_recv_tokens, num_tokens, num_experts = handle
            recv_x, recv_x_scales, _, _, _, _, _, _, event = \
                self.runtime.intranode_ace_dispatch(
                    x, x_scales, None, None,
                    None, None,
                    row_id_map_non_trans, global_stride_list, num_recv_tokens,
                    num_experts,
                    expert_alignment, buffer_idx,
                    getattr(previous_event, 'event', None), async_finish, allocate_on_comm_stream)
            return (recv_x, recv_x_scales) if x_scales is not None else recv_x, None, None, None, None, EventOverlap(event)
            
        recv_x, recv_x_scales, recv_topk_idx, recv_topk_weights, num_recv_tokens_per_expert_list, row_id_map_non_trans, global_stride_list, num_recv_tokens, event = \
            self.runtime.intranode_ace_dispatch(
        ...
        handle = (global_stride_list, row_id_map_non_trans, num_recv_tokens, x.size(0), num_tokens_per_expert.size(0))
    ...
    def combine_ace(self, x: torch.Tensor, handle: Tuple, topk_weights: Optional[torch.Tensor] = None, previous_event: Optional[EventOverlap] = None, async_finish: bool = False, allocate_on_comm_stream: bool = False):
        """
        Combine tokens using ACE.
        unpermute is now done in C++ for better performance.
        """
        global_stride_list, row_id_map_non_trans, num_recv_tokens, num_tokens, num_experts = handle
        
        recv_x, output, combined_topk_weights, event = self.runtime.intranode_ace_combine(x, topk_weights, global_stride_list, row_id_map_non_trans, num_tokens, num_experts, getattr(previous_event, 'event', None), async_finish, allocate_on_comm_stream)
    ...
    def dispatch(self, x: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
                 ...
                 previous_event: Optional[EventOverlap] = None, async_finish: bool = False,
                 allocate_on_comm_stream: bool = False, ace_buffer_idx: int = 0) -> \
        ...
        if self.use_ace:
            # assert topk_idx is not None and topk_weights is not None, 'ACE dispatch requires topk_idx and topk_weights'
            return self.dispatch_ace(x, handle, topk_idx, topk_weights, num_tokens_per_rank, num_tokens_per_expert,  expert_alignment, previous_event, async_finish, allocate_on_comm_stream, ace_buffer_idx)
    ...
    def combine(self, x: torch.Tensor, handle: Tuple,
        ...
        if self.use_ace:
            combined_x, combined_topk_weights, event = self.combine_ace(x, handle, topk_weights, previous_event, async_finish, allocate_on_comm_stream)
