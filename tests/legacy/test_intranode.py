...
from deep_ep.utils.math import calc_diff, inplace_unique, per_token_cast_to_fp8, per_token_cast_back, hash_tensor
        ...
        for async_mode in (False, True):
            for current_x in filter(lambda elem: elem is not None, (x_pure_rand, x, x_e4m3)):
                for with_topk in (False, True):
                    ...
                    if not with_topk and use_ace == False:
                    ...
                    # Test combine
                    if use_ace == True:
                        ace_recv_x, _ = buffer.get_ace_combine_buffer(recv_x.size(0), recv_x.size(1), 1, with_topk)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--use-ace", action='store_true', help='Whether dispatch/combine use ace')
