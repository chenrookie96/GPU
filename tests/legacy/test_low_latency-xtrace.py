...
def plot_deepxtrace_heatmap(
    ...
    cmap = LinearSegmentedColormap.from_list("optimized_ryg", colors)
...
def test_main(num_tokens: int,
    ...
    dispatch_wait_recv_cost_stats = torch.zeros((num_ranks, ), dtype=torch.int64, device='musa')
    combine_wait_recv_cost_stats = torch.zeros((num_ranks, ), dtype=torch.int64, device='musa')
    # noinspection PyShadowingNames
    ...
    print(
        ...
        flush=True)
    all_dispatch_wait_recv_cost_stats = torch.empty((num_ranks, num_ranks), dtype=torch.int64, device='musa')
    dist.all_gather_into_tensor(all_dispatch_wait_recv_cost_stats, dispatch_wait_recv_cost_stats, group=group)
    all_combine_wait_recv_cost_stats = torch.empty((num_ranks, num_ranks), dtype=torch.int64, device='musa')
    dist.all_gather_into_tensor(all_combine_wait_recv_cost_stats, combine_wait_recv_cost_stats, group=group)
    if rank == 0:
        plot_deepxtrace_heatmap(all_dispatch_wait_recv_cost_stats.cpu().numpy(), file_name="dispatch", title="Dispatch Heatmap")
        plot_deepxtrace_heatmap(all_combine_wait_recv_cost_stats.cpu().numpy(), file_name="combine", title="Combine Heatmap")
