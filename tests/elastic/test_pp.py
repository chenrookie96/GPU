...
def generate_stress_ops(rank_idx: int, num_ranks: int, num_sends: int, shape: tuple):
    ...
    for (src_rank_idx, dst_rank_idx) in send_times:
        ...
        for i in range(n):
            tensor = torch.randn(shape, dtype=torch.bfloat16, device='musa')
...
def test(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    for seed in range(args.num_stress_iterations):
        ...
        for j, (op, timestamp, peer, _, tensor) in enumerate(ops):
            ...
            if timestamp > prev:
                torch.musa._sleep(int((timestamp - prev) / 10 ** 8 * args.num_sleep_cycles))
            prev = timestamp
    ...
    for hide_rdma_latency in (True, False):
        for num_concurrent in (1, 2, 3):
            send_tensors = [torch.randn(shape, dtype=torch.bfloat16, device='musa') for _ in range(num_concurrent)]
            recv_tensors = [torch.empty(shape, dtype=torch.bfloat16, device='musa') for _ in range(num_concurrent)]

            def loop(_hide_rdma_latency=hide_rdma_latency):
                torch.zeros((131072, 32768), dtype=torch.int, device='musa')
                for t in send_tensors:
                    buffer.pp_send(t, (rank_idx + 1) % num_ranks)
                if _hide_rdma_latency:
                    torch.musa._sleep(num_approx_rdma_cycles * num_concurrent)
