...
from deep_ep.utils.refs import generate_pre_combine_data, ordered_accumulate, index_rows, equal_values
...
def fold_expanded(expanded: Union[Tuple[torch.Tensor], torch.Tensor],
    ...
    gathered = index_rows(expanded, indices)
    first_valid_idx = valid_mask.to(torch.int).argmax(dim=1)
    folded = index_rows(gathered, (torch.arange(gathered.shape[0], device='musa'), first_valid_idx))
    result = equal_values(gathered, folded.unsqueeze(1)).all(dim=-1)
...
def test_dispatch_combine(buffer: deep_ep.ElasticBuffer, args: argparse.Namespace):
    ...
    for (do_handle_copy, expert_alignment, use_fp8_dispatch, num_bias,
        ...
        bias = torch.randn((num_tokens, hidden), dtype=torch.bfloat16, device='musa') if num_bias == 1 else None
        if num_bias == 2:
            bias = tuple(torch.randn((num_tokens, hidden), dtype=torch.bfloat16, device='musa') for _ in range(num_bias))
        ...
        if not args.skip_check:
            ...
            ref_y = generate_pre_combine_data(
                dist.get_rank() * num_max_tokens_per_rank + torch.arange(num_tokens, device='musa'),
        ...
        input_for_combine = torch.empty_like(recv_x_bf16, dtype=torch.bfloat16, device='musa')
        ...
        input_for_expand_combine = torch.empty((expanded_recv_x_bf16.shape[0] + 1, hidden), dtype=torch.bfloat16, device='musa')
        ...
        if not args.skip_check:
            ...
            # Cached checks
            if use_fp8_dispatch:
                assert equal_values(recv_x[0], cached_recv_x[0]).all()
            ...
            for check_recv_x, check_recv_topk_idx, check_recv_topk_weights, check_handle in (
                ...
                for i in range(buffer.num_ranks):
                    ...
                    for ref_t, t, do_mask in check_list:
                        ...
                        t = index_rows(t, sorted_indices)
                        if do_mask:
                            ref_t = ref_t.masked_fill(ref_mask, 0)
                            t = t.masked_fill(ref_mask, 0)
                        assert equal_values(ref_t, t).all(), f'{ref_t=}, {t=}'
