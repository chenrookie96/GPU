...
def all_gather_ref(shape: tuple, rank_idx: int, num_ranks: int, round_idx: int = 0):
    ...
    for i in range(num_ranks):
        ...
        ref_list.append(torch.randn(shape, dtype=torch.bfloat16, device='musa'))
...
def do_all_gather(buffer: deep_ep.ElasticBuffer,
                  ...
                  start_event: torch.musa.Event | None = None):
    ...
    if start_event is not None:
        torch.zeros(int(256e6 // 4), dtype=torch.int, device='musa')  # flush L2 cache
        start_event.record()
...
def test(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    for num_bytes in (2 ** p for p in range(20, 27)):
        # Create tensors
        shape = (num_bytes // 2, )
        tensors = tuple(torch.randn(shape, dtype=torch.bfloat16, device='musa') for _ in range(num_max_inflight_agrs))
