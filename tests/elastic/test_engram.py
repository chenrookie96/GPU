...
def test(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    if num_qps == 0:
        num_qps = torch.musa.get_device_properties('musa').multi_processor_count
    ...
    local_bf16 = torch.randn((args.num_entries, args.hidden), dtype=torch.bfloat16, device='musa')
    local_sf = None
    if args.use_fp8:
    ...
    sf = None
    if args.use_fp8:
        sf = torch.empty((num_ranks * args.num_entries, local_sf.shape[1]), dtype=local_sf.dtype, device='musa')
    ...
    indices = torch.randint(0, num_ranks * args.num_entries,
                            (args.num_tokens, args.num_entries_per_token), device='musa', dtype=torch.int)

    # Correctness check
    if not args.skip_check:
        global_storage = torch.empty((num_ranks * args.num_entries, args.hidden), dtype=dtype, device='musa')
